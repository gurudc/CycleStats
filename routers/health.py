...
@router.get("/checkins")
def list_checkins(days: int = 90, db: Session = Depends(get_db)):
    """Get health check-in history."""
    return get_checkins(db, days)


@router.post("/checkins")
def create_or_update_checkin(data: dict, db: Session = Depends(get_db)):
...
@router.get("/hrv")
def hrv_trend(days: int = 30, db: Session = Depends(get_db)):
    """Get HRV trend data."""
    return compute_hrv_trend(db, days)


@router.get("/resting-hr")
def resting_hr_trend(days: int = 90, db: Session = Depends(get_db)):
    """Get resting heart rate trend."""
    return compute_resting_hr_trend(db, days)


@router.get("/sleep")
def sleep_trend(days: int = 90, db: Session = Depends(get_db)):
    """Get sleep trend data."""
    return compute_sleep_trend(db, days)


@router.get("/recovery")
def recovery_trend(days: int = 30, db: Session = Depends(get_db)):
    """Get recovery and body battery trends."""
    return compute_recovery_trend(db, days)


@router.get("/vo2max")
def vo2max_estimate(db: Session = Depends(get_db)):
